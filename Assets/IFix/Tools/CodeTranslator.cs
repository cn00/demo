    ...
    class CodeTranslator
    {
            ...
            Dictionary<Instruction, int> ilOffset, out int stopPos)
        {
            ...
            for (int i = 0; i < instructions.Count; i++)
            {
                ...
                stopPos = i;
                //Debug.Log(i + " instruction:" + instructions[i].OpCode + " offset:" + offset);
                    ...
                    case Code.Ldvirtftn:
                        {
                                ...
                                && !isCompilerGeneratedByNotPlainObject(mr.DeclaringType))
                            {
                                    ...
                                    || isCustomClassPlainObject(mr.DeclaringType))
                                {
                                    ...
                                    {
                                        //Debug.Log("check " + md + " fail il = " + md.Body.Instructions[p]
                                        //    + ",caller=" + method);
                                        return false;
                                    }
                                    //编译器生成类要检查所有实现方法
        ...
        {
            //Debug.Log("callee:" + callee + ", caller:" + caller);
            ...
            {
                Debug.Log("Warning: unsafe method, " + method + " in " + method.DeclaringType);
            ...
            //Debug.Log("process method id:" + codes.Count);
            ...
            //    || methodToInjectType[method] == InjectType.Redirect)
            {
                ...
                if (!checkILAndGetOffset(method, msIls, ilOffset, out stopPos))
                {
                    ...
                    if (methodToInjectType.TryGetValue(method, out it))
                    {
                        ...
                        {
                            Debug.Log("Warning: not support il[" + msIls[stopPos] + "] in " + method
                    ...
                    {
                        Debug.Log("Info: not support il[" + msIls[stopPos] + "] in " + method
                            + (caller == null ? "" : (", caller is " + caller)));
                    }
            ...
            try
            {
                ...
                for (int i = 0; i < body.ExceptionHandlers.Count; i++)
                {
                    ...
                    //Debug.Log("---------------" + i + "---------------");
                    //Debug.Log("HandlerType:" + exceptionHandlers[i].HandlerType);
                    //Debug.Log("CatchTypeId:" + exceptionHandlers[i].CatchTypeId);
                    //Debug.Log("TryStart:" + exceptionHandlers[i].TryStart);
                    //Debug.Log("TryEnd:" + exceptionHandlers[i].TryEnd);
                    //Debug.Log("HandlerStart:" + exceptionHandlers[i].HandlerStart);
                    //Debug.Log("HandlerEnd:" + exceptionHandlers[i].HandlerEnd);
                }
                ...
                {
                    var msIl = msIls[i];
                    //Debug.Log("msIl:" + msIl.OpCode.Code + ", idx:" + code.Count);
                    ...
                    switch (msIl.OpCode.Code)
                    {
                        ...
                        case Code.Blt_Un_S:
                            ...
                            //{
                            //    Debug.Log("il:" + msIl + ",jump to:" + msIl.Operand);
                            //    Debug.Log("il pos:" + ilOffset[msIl] + ",jump to pos:" 
                        ...
                        case Code.Ldvirtftn:
                            {
                                    ...
                                    && (isCompilerGeneratedPlainObject(methodToCall.DeclaringType) || isCustomClassPlainObject(methodToCall.DeclaringType))) // closure
                                {
                                    ...
                                    //Debug.Log("closure: " + methodToCall);
                                ...
                                {
                                    //Debug.Log("loadftn for static: " + methodToCall);
                                    getWrapperMethod(wrapperType, anonObjOfWrapper, methodToCall as MethodDefinition,
                        ...
                        case Code.Ldflda:
                            {
                                ...
                                if (isCompilerGenerated(field.DeclaringType) || isCompilerGeneratedPlainObject(field.DeclaringType) || isCustomClassPlainObject(field.DeclaringType))
                                {
                                    ...
                                    //Debug.Log("anon obj field:" + field + ",idx:" +
            ...
            catch(Exception e)
            {
                ...
                {
                    Debug.Log("Warning: process " + method + " il throw " + e);
                }
                else
                {
                    throw e;
            ...
            //Debug.Log("process finish:" + method);
            if (mode == ProcessMode.Inject)
            {
        ...
        void addInterfacesOfTypeToBridge(TypeDefinition anonType)
        {
            ...
            foreach (var method in anonType.Methods.Where(m => !m.IsConstructor))
            {
                ...
                if (matchItfMethod != null)
                {
                    ...
                    //Debug.Log("add slot " + matchItfMethod + ",m=" + method);
                ...
                else
                {
                    ...
                    else
                    {
                        if (paramRawType.IsValueType)
                        {
                            ...
                            //Debug.Log("Call_PushValueType_Ref for " + method.Name + ", pidx:" + i
        ...
        void writeMethod(BinaryWriter writer, MethodReference method)
        {
            ...
            if (method.IsGenericInstance)
            {
                //Debug.Log("GenericInstance:" + externMethod);
                ...
                //Debug.Log(">>>" + ((GenericInstanceMethod)externMethod).GetElementMethod());
                //Debug.Log(((GenericInstanceMethod)externMethod).ElementMethod.HasGenericParameters);
                //foreach (var gp in genericParameters)
                //{
                //    Debug.Log("gp:" + gp + ",gpdm:" + gp.Type);
                //}
                foreach (var p in method.Parameters)
                {
                    ...
                    if (paramIsGeneric)
                    {
                        ...
                        //    }
                        //}
                        //Debug.Log("p.ParameterType:" + p.ParameterType.FullName + ",isg:"
            ...
            {
                //Debug.Log("not GenericInstance:" + externMethod);
                ...
                foreach (var p in method.Parameters)
                {
                    ...
                    //Debug.Log("paramType: " + paramType);
                    //Debug.Log("externTypeToId[paramType] : " + externTypeToId[paramType]);
                    //Debug.Log("externTypes[externTypeToId[paramType]] : "
        ...
        void writeSlotInfo(BinaryWriter writer, TypeDefinition type)
        {
            ...
            foreach(var ii in type.Interfaces)
            {
                ...
                foreach (var method in itfDef.Methods)
                {
                    ...
                    if (implMethod == null)
                    {
                        //Debug.Log(string.Format("check {0} in {1}", itfMethod, type));
                        //foreach(var cm in type.Methods)
                        //{
                        //    Debug.Log(string.Format("{0} {1}", cm, itfMethod.CheckImplemention(cm)));
                        //}
                        throw new Exception(string.Format("can not find method {0} of {1}", itfMethod, itf));
                    }
                    //Debug.Log(string.Format(">>>{0} [{1}]", itfMethod, methodToId[implMethod]));
        ...
        public void Serialize(Stream output)
        {
            using (BinaryWriter writer = new BinaryWriter(output))
            {
                ...
                {
                    //Debug.Log("externTypes[" + i + "]: " + externTypes[i] + ", is ref:"
                ...
                for (int i = 0; i < nextAllocId; i++)
                {
                    ...
                    if (codes.TryGetValue(i, out code) && codeMustWriteToPatch.Contains(i))
                    {
                        ...
                        //Debug.Log("methodid=" + i + ",instruction.cout=" + codes[i].Count);
                        for (int j = 0; j < code.Count; j++)
                        {
                            writer.Write((int)code[j].Code);
                            writer.Write(code[j].Operand);
                            //Debug.Log(j + " :code = " + codes[i][j].Code + ", operand="
                            //    + codes[i][j].Operand);
                        }
                    ...
                    {
                        //Debug.Log("no code for methodId:" + i);
                        writer.Write(0);
                        writer.Write(0);
                    }
                }
                //Debug.Log("nextAllocId:" + nextAllocId + ", realCodeNumber:" + realCodeNumber);
                //Debug.Log("serialize extern method...");
                ...
                //Debug.Log("serialize anonymous type...");
                writer.Write(anonymousTypeInfos.Count);
                for(int i = 0; i < anonymousTypeInfos.Count; i++)
                {
                    //Debug.Log("anonymous type: " + anonymousTypeInfos[i]);
                ...
                //Debug.Log("interpretMethods.Count:" + interpretMethods.Count);
                //int idx = 0;
                foreach (var kv in interpretMethods)
                {
                    //Debug.Log((idx++) +" method:" + kv.Key + ", id:" + kv.Value);
            ...
            //    select method))
            //{
            //    Debug.Log("hgp:" + method + ",type:" + method.GetType());
            //}
        }
