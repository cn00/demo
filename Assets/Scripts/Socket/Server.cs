...
namespace Net
{
    ...
    public partial class Server
    {
        ...
        , ProtocolType ProtocolType = ProtocolType.Tcp)
        {
            ...
            this._socket = new Socket(AddressFamily.InterNetwork, SocketType, ProtocolType);
        ...
        {
            return _clients.Find(i => i.GetGuid() == Guid);
        }
        ...
        {
            return _clients.Find(i => i.GetSocket() == Socket);
        }
        ...
        private void start()
        {
            GetSocket().Blocking = true;//false;//

            // Bind the socket and start listending
            GetSocket().Bind(GetEndPoint());

            if(GetSocket().ProtocolType == ProtocolType.Tcp)
            {
                GetSocket().Listen(64);
        ...
        private void connectionCallback(IAsyncResult AsyncResult)
        {
            try
            {
                ...
                // if(handshakeReceived < handshakeBuffer.Length)
                //     Array.Resize<byte>(ref handshakeBuffer, handshakeReceived);
                // var pasword = Encoding.Default.GetString(handshakeBuffer).TrimEnd(new []{'\n', '\r', ' ', '\0'});
                // if(pasword != "654123")
                // {
                //    goto again;
                // }
        ...
        {
            client.GetSocket().Send(mCacheMsg.SelectMany(i => i).ToArray());
        }
