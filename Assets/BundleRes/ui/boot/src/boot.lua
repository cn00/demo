...
boot.coroutine1 = coroutine.create(function()
    ...
    yield_return(CS.AssetHelper.Instance:GetAsset("ui/loading/loading.prefab", function(asset)
        obj = asset;
    end))
    CS.AppLog.d("lua AppBoot.Start {0}", obj);
    ...
    yield_return(CS.AssetHelper.Instance:GetAsset("ui/login/login.prefab", function(asset)
        obj = asset;
    end))
    CS.AppLog.d("lua login {0}", obj);
    ...
    local www = CS.UnityEngine.WWW('http://cn210100707.local:8008')
    yield_return(www)
