...
local this = {}
--[[
    var lbytes = File.ReadAllBytes("iospatch/PatchXcodeProject.lua");
...
function this:Patch( argv )
	local buildTarget = argv[0]
	local pathToBuiltProject = argv[1]
...
function this.UpdateBuildProperty(targetGuid, key, v )
    ...
    if (type(v) == "string") then
        pbxProject:SetBuildProperty(targetGuid, k, v);
    elseif (type(v) == "table") then
    	...
    	pbxProject:UpdateBuildProperty(targetGuid, k, this.Table2CSArray(v), nil)
    end
end
